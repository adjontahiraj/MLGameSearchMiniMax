...
class BoxesandGridsGame():
    ...
    def player2(self):
        ...
        score, next_move=self.minimax(self.boardh,self.boardv, 3, -float("inf"),float("inf"),True,0,self.score_player1, self.score_player2) 
    ...
    def minimax(self,horizontal,vertical, depth, alpha, beta, maxPlayer, move, score1, score2):
        #check if there are any moves left or the depth is 0
        if depth == 0 or bg.game_ends(horizontal, vertical):
            return self.evaluate(score1, score2),None

        #check if it is the max player
        if maxPlayer == True:
            hval = -float("inf")
            idealMove = None
            next_move = self.list_possible_moves(horizontal, vertical)

            #traversing the moves
            for i in next_move:
                #getting the next state
                state_h, state_v, score = self.next_state(i, horizontal, vertical)
                #recursing into the next depth
                newHVal, newIdealMove = self.minimax(state_h, state_v, depth -1, alpha, beta, False, i, score1, score2+score)
                #checking if new val is better
                if newHVal > hval:
                    hval = newHVal
                    idealMove = i
                #getting max alpha
                alpha = max(alpha, hval)
                #pruning but orint commented out to make it run faster as instructed
                if alpha >= beta:
                    #beta pruning
                    #print("beta prune ", beta)

                    break

            return hval, idealMove
        else:
            hval = float("inf")
            idealMove = None
            ...
            for i in next_move:
                ...
                state_h, state_v, score = self.next_state(i, horizontal, vertical)
                #recursing into the next depth
                newHVal, newIdealMove = self.minimax(state_h, state_v, depth -1, alpha, beta, True, i, score1+score, score2)
                #checking the new values
                if newHVal < hval:
                    hval = newHVal
                    idealMove = i
                #picking smaller beta
                beta = min(beta, hval)
                #pruning print commented out as instructed for faster run
                if alpha >= beta:
                    #alpha pruning
                    #print("alpha prune ", alpha)
                    break
            return hval, idealMove

    #implemented in the minmax
    ...
    my strategy is just to get the diference of the two scores
    '''
    def evaluate(self,score1, score2):
